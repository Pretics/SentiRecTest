...
def prep_news_dataset(
    ...
    processed_news = []
    news_collection = get_news_dataset(args)
    news2int = {}
...
def prep_news(args, embeddings: dict):
    ...
    1. sentiment classifier를 정의하고, category2int, word2int, embedding_weights를 불러오거나 생성합니다.
    2. news.tsv 모든 뉴스 데이터에 대해 반복문을 돌아 news2int, category2int를 생성합니다.
    3. title, abstract을 고정 길이의 인덱스로 변환합니다.
    4. title을 이용해 (vader, bert) sentiment score를 구합니다.
    5. 생성한 모든 데이터를 parsed_news.tsv에 저장합니다.
    """
    if args.mode == "train": 
        category2int = {}
        word2int = {}
    ...
    with open(path.join(args.out_dir, 'parsed_news.tsv'), 'w', newline='') as news_file:  
        ...
        processed_newsdata = prep_news_dataset(args, embeddings, category2int, word2int, embedding_weights)
