...
python "$preprocessDir/parse_behavior_combined.py" --train-file "$datasetTrainDir/behaviors.tsv" --test-file "$datasetTestDir/behaviors.tsv" --train-out "$preTrainDir" --test-out "$preTestDir" --user2int "$preTrainDir/user2int.tsv"

Write-Output "Preprocessing training/test-set news content..."
python "$preprocessDir/parse_news_combined.py" --train-file "$datasetTrainDir/news.tsv" --test-file "$datasetTestDir/news.tsv" --train-out "$preTrainDir" --test-out "$preTestDir" --word-embeddings "$wordEmbeddingDir/glove.840B.300d.txt"
